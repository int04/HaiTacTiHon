...
module.exports = function(socket,data) {
    ...
    if(lo_chua_duc <= 0) return socket.sendCode(-61);

    let tien = config.duclo[lo_chua_duc];
    if(my.tien.ruby < tien) return socket.sendCode(-62);
    ...
    itemData.forEach(e => {
        ...
        if(bag) {
            ...
            if(infoItem) {
                ...
                if(infoItem.type === 'item' && infoItem.type2 === 'duclo') {
                    idItem = bag.id;
                    idItemData = bag.item;
                }
            }
    ...
    if(sai) return socket.sendCode(-63);
    ...
    init04.log(my.id, 'duclo', history);
