import io_Object from '../../_io/object.js';
import confirm from "./confirm.js";
export default class NoticeView extends io_Object {
    constructor () {
        super();
    }

    confirm  = (text, callOK, callCancel) => {
        return confirm(this,text, callOK, callCancel);
    }

    deleteNotice = () => {
        let background = this.noticeMain.getChildByName("background");
        if(background) {
            // fade use tween
            TweenMax.to(background, 0.5, {
                alpha: 0,
                onComplete: () => {
                    this.noticeMain.visible = false;
                    this.noticeMain.removeChildren();
                }
            });
        } else {
            this.noticeMain.visible = false;
    ...
    wait = (text) => {
        return this.notice(text,false);
    }

    notice = (text,callout = true) => {
        this.noticeMain.removeChildren();
        this.noticeMain.visible = true;
        ...
        this.noticeMain.addChild(background);
        background.interactive = true;

        let width = this.gameWidth * 0.7;
        width = width > 400 ? 400 : width;

        let hegit = this.gameHeight * 0.3;
        ...
        let ui = this.createUI(width,hegit, {
        ...
        ui.y = (this.gameHeight*0.8 - ui.height);
