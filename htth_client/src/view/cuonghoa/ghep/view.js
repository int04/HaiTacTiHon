...
let resetItem = (self) => {
    let list = self.cache.ghepItem.data;
    list.forEach(id => {
    ...
    let select = self.cache.ghepItem.id;
    if(select !==0) {
        let bag = self.my.ruong.data.find(e => e.id === select && e.active === 'hanhtrang');
        if(!bag) {
            self.cache.ghepItem.id = 0;
            self.cache.ghepItem.data = [];
        }
    }
}
...
let checkIndex = (self) => {
    ...
    for(let i = 0; i < self.cache.ghep.length; i++) {
        let element = self.cache.ghep[i];
        let itemTo = element[0];
        let tien = element[1];
        let itemNeed = element[2];
        if(itemTo && tien && itemNeed) {
            if(list.length !== itemNeed.length) continue;
            let co = 0;

            itemNeed.forEach(idvpNeed => {
                let getIdVP = idvpNeed[0];
                if(getIdVP) {
                    list.forEach(idSelect => {
                        let inBag = self.my.ruong.data.find(e => e.id === idSelect && e.active === 'hanhtrang');
                        if(inBag) {
                            if(inBag.item === getIdVP) {
                                co++;
                            }
                        }
                    })
            ...
            if(co === list.length) index = i;
        }
    }

    self.cache.ghepItem.id = index;
}
...
const view = (self, id) => {
    ...
    let ui1 = created_ui_1(self,[
        [9208,"shop","Ghép vật phẩm",id,view],
    ...
    let preview = self.cache.ghepItem.data;
    ...
    if(body) {
            ...
            if(data[i]) {
                ...
                o.on('pointerup', () => {
                    if(Date.now() - time < 200) {
                        self.previewItemBag(o,data[i],'btn_ghep');
                    }
        ...
        for(let i = 0; i < num_box; i++) {
            ...
            if(self.cache.ghepItem.id != -1) {
                let index = self.cache.ghepItem.id;
                let ghep = self.cache.ghep[index];
                if(ghep) {
                    let itemTo = ghep[0];
                    if(itemTo) {
                        let idItemTo = itemTo[0];
                        let tile = itemTo[1];
                        let phamchat = itemTo[2];
                        let json = {
                            id : 'int04',
                            time : Date.now(),
                            soluong : 1,
                        ...
                        animation_run.name = "animation_run_ghep";
                        ...
                        animation_success.name = "animation_success_ghep";
        ...
        let text = new PIXI.Text(self._('Hợp nhất'),{fontFamily : 'Arial', fontSize: 14, fill : 0xffffff, align : 'center'});
        text.anchor.set(0.5);
        ...
        if(self.cache.ghepItem.id != -1) {
            ...
            if(ghep) {
                ...
                if(itemTo) {
                    ...
                    let tile = itemTo[1];
                    let phamchat = itemTo[2];
                    let baseItemGhep = self.item.find(e => e.id === idItemTo);
                    let y = 0;
                    if(baseItemGhep) {
                        let text = Text(self,'Để ghép vật phẩm này cần:',right);
                        text.y = y;
                        ...
                        if(tien) {
                            ...
                            let ruby = tien[1];
                            if(beri && beri >=1) {
                                let text = Text(self,'- '+beri+' Bery',right);
                                text.y = y;
                                containerText.addChild(text);
                                y += text.height + 5;
                            }
                            if(ruby && ruby >=1) {
                                let text = Text(self,'- '+ruby+' Ruby',right);
                                text.y = y;
                                containerText.addChild(text);
                                y += text.height + 5;
                            }
                        ...
                        if(itemNeed) {
                            itemNeed.forEach(e => {
                                ...
                                let soluong = e[1];
                                let hoantra = e[2];
                                let baseItem = self.item.find(e => e.id === id);
                                if(baseItem) {
                                    let text = Text(self,'- '+baseItem.name+' x'+soluong,right);
                                    text.y = y;
                                    containerText.addChild(text);
                                    y += text.height + 5;
                                }
                        ...
                        if(tile < 100) {
                            let text = Text(self,'- '+tile+'% tỉ lệ thành công',right);
                            text.y = y;
                            containerText.addChild(text);
                            y += text.height + 5;
                        }
            ...
            let text1 = Text(self,'Lựa chọn các vật phẩm, sau đó bấm ghép để tiếp tục.',right);
