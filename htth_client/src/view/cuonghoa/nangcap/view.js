...
let resetItem = (self) => {
    let list = self.cache.nangcap.data;
    list.forEach(id => {
...
const view = (self, id) => {
    ...
    let preview = self.cache.nangcap.data;
    ...
    if(body) {
            ...
            if(data[i]) {
                ...
                o.on('pointerup', () => {
                    if(Date.now() - time < 200) {
                        self.previewItemBag(o,data[i],'btn_nangcap');
                    }
        ...
        for(let i = 0; i < num_box; i++) {
            ...
            let idItem = self.cache.nangcap.id;
            if(idItem) {
                let event = false;
                ...
                if(!item) {
                    ...
                    if(!infoItem || infoItem && infoItem.type !== 'trangbi') {
                        ...
                        if(i === 0) {
                            ...
                            let tontai = self.config.nangcap.tile[createData.level];
                            if(tontai) {
                                let newData = Object.assign({},createData);
                                newData.level = createData.level + 1;
                ...
                if(event === true) {
                    ...
                    o.on('pointerup', () => {
                        if(Date.now() - time < 200) {
                            self.previewItemBag(o,createData,'btn_nangcap');
                        }
                    });
                }
        ...
        let text = new PIXI.Text(self._('Nâng cấp'),{fontFamily : 'Arial', fontSize: 14, fill : 0xffffff, align : 'center'});
        text.anchor.set(0.5);
        ...
        if(self.cache.nangcap.id) {
            let bag = my.ruong.data.find(e => e.id === self.cache.nangcap.id && e.active === 'hanhtrang');
            if(bag) {
                let level = bag.level;
                let config = self.config.nangcap;
                let tile = config.tile[level];
                if(!tile) {
                    let text1 = Text(self,'Vật phẩm đã đạt cấp độ tối đa.',right);
                    containerText.addChild(text1);
                }
                else {
                    let text1 = Text(self,'Để nâng cấp trang bị lên cấp '+(level+1)+' cần: ',right);
                    containerText.addChild(text1);
                    let beri = config.beri;
                    let da = config.da;
                    beri = beri[level];
                    da = da[level];
                    let text2 = Text(self,'- '+beri+' Beri',right);
                    text2.y = text1.y + text1.height + 5;
                    containerText.addChild(text2);
                    let text3 = Text(self,'- '+da+' đá',right);
                    text3.y = text2.y + text2.height + 5;
                    containerText.addChild(text3);

                    // check xem có vp tăng tỉ lệ không
                    let tangtile = 0;
                    self.cache.nangcap.data.forEach(idvp => {
                        let checkbag = my.ruong.data.find(e => e.id === idvp && e.active === 'hanhtrang');
                        if(checkbag) {
                            let baseItem = self.item.find(e => e.id === checkbag.item);
                            if(baseItem) {
                                if(baseItem.type2 === 'nangcaotile') {
                                    tangtile = baseItem.value;
                                }
                            }
                        }
                    })

                    let text4 = Text(self,'- '+tile+'%'+(tangtile > 0 ? ' +'+tangtile+'%' : '')+' tỉ lệ thành công',right);
                    text4.y = text3.y + text3.height + 5;
                    containerText.addChild(text4);
                }
            }
        }
        else {
            let text1 = Text(self,'Lựa chọn vật phẩm cần nâng cấp, sau đó chọn đá.',right);
            containerText.addChild(text1);
        }
