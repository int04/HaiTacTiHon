...
let resetItem = (self) => {
    let list = self.cache.duclo.data;
    list.forEach(id => {
        let bag = self.my.ruong.data.find(e => e.id === id && e.active === 'hanhtrang');
        if(!bag) {
            self.cache.duclo.data = self.cache.duclo.data.filter(e => e !== id);
        }
        else {
...
const view = (self, id) => {
    ...
    let preview = self.cache.duclo.data;
    ...
    if(body) {
        ...
        for(let i = 0; i < num_box; i++) {
            ...
            let idItem = self.cache.duclo.id;
            if(idItem) {
                let event = false;
                ...
                if(!item) {
                    ...
                    if(!infoItem || infoItem && infoItem.type !== 'trangbi') {
                        ...
                        if(i === 0) {
                            ...
                            let giatien = self.config.duclo[lo_chua_duc];
                            if(giatien && giatien !== -1) {
                ...
                if(event === true) {
                    ...
                    o.on('pointerup', () => {
                        if(Date.now() - time < 200) {
                            self.previewItemBag(o,createData,'btn_duclo');
                        }
                    });
                }
        ...
        let text = new PIXI.Text(self._('Đục ngay'),{fontFamily : 'Arial', fontSize: 14, fill : 0xffffff, align : 'center'});
        text.anchor.set(0.5);
        ...
        if(self.cache.duclo.id) {
            let bag = my.ruong.data.find(e => e.id === self.cache.duclo.id && e.active === 'hanhtrang');
            if(bag) {
                let level = bag.level;
                ...
                let giatien = self.config.duclo[lo_chua_duc];
                if(!giatien || giatien === -1) {
                    let text1 = Text(self,'Vật phẩm này đã có đủ lỗ.',right);
                    containerText.addChild(text1);
                }
                else {
                    let text1 = Text(self,'Để đục thêm 1 lỗ cho trang bị này cần: ',right);
                    containerText.addChild(text1);
                    let text2 = Text(self,'- '+giatien+' ruby',right);
                    text2.y = text1.y + text1.height + 5;
                    containerText.addChild(text2);
                    let text3 = Text(self,'- 1 búa đục lỗ',right);
            ...
            let text1 = Text(self,'Lựa chọn vật phẩm cần đục lỗ, sau đó chọn búa để tiến hành đục.',right);
            containerText.addChild(text1);
        }
