const selectItem = (self, id) => {
    ...
    if(infoItem.type === 'trangbi' && self.cache.duclo.id !== 0) {
        self.cache.duclo.data = self.cache.duclo.data.filter(e => e !== self.cache.duclo.id);
    ...
    if(infoItem.type === 'item') {
        if(infoItem.type2 !== 'duclo') return self.chipi(self._('Vật phẩm này không thể nâng cấp'));
    }

    // check xem tồn tại loại vp này chưa
    let vipham = false;
    self.cache.duclo.data.forEach(idvp=> {
    ...
    if(infoItem.type === 'trangbi') {
        self.cache.duclo.id = id;
    }
    self.cache.duclo.data.push(id);
    return self.duclo();
...
const checkList = (self, id) => {
    if(self.cache.duclo.data.find(e => e === id)) return true;
    return false;
}

const btn_duclo = (self, id) => {
